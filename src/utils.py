...
def add_padding(img, psize, val=0):
    ...
    try:
        ...
        npad_img = ((psize//2+1, psize//2+1), (psize//2+1, psize//2+1), (0,0))
    except:
        bands = 0
        row, col = img.shape
...
def path_by_prop(path, prop, names=["train"]):
    list_files = []
    list_class = []
    for n in names :
        subfold = os.listdir(os.path.join(path,n))
        for f in range(len(subfold)):
            files = np.array(glob.glob(os.path.join(path,n,subfold[f])+ "/*.png"))
            index = np.random.choice(len(files), int(round(prop[f]*len(files))), replace=False)
            list_files.extend(files[index])
            list_class.extend([f]*len(index))
        
    return list_files, list_class
        

def proportion_compute_all(label_input):

    gen_ohe = OneHotEncoder()
    total = len(label_input)
    label_input = label_input.reshape(len(label_input), 1)
    label_input = gen_ohe.fit_transform(label_input).toarray()
    prob_result = label_input.sum(axis = 0)/total
    return  prob_result

def proportion_compute(label_input, num_classes):

    gen_ohe = F.one_hot(label_input, num_classes=num_classes)
    total = len(label_input)
    prob_result = gen_ohe.sum(axis = 0)/total
    return  prob_result
